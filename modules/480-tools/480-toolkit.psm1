...
function defaults(){
    ...
    Write-Host "vCenter: "$conf.vcenter_server
    Write-Host "ESXi Host: "$conf.esxi_host
    Write-Host "Folder: "$conf.folder
    Write-Host "Network: "$conf.default_network
    Write-Host "Datastore: "$conf.default_datastore
    Write-Host ""
    $userDef = Read-Host "Would you like to change the default configurations (Y/N)"
    if ($userDef -eq "Y" -or $userDef -eq "y" -or $userDef -eq "yes"-or $userDef -eq "Yes") {
        Write-Host $conf
        # TODO: Provide prompt to change defaults, including default in brackets
    } else {
        Write-Host ""
        Write-Host -ForegroundColor "Green" "Using Default Configuration"
        Write-Host ""
        intMenu
    }
}
